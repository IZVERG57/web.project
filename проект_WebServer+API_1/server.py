...
def converting():
    ...
    # Краткие символы валют
    curr_in_short = curr_in[:3]
    curr_out_short = curr_out[:3]

    # Исходное состояние, отсутствие данных
    if value is None:
        value = '0'
    ...
    try:
        ...
        result = 'Данные по валюте не найдена'

    # Значение единицы валюты
    try:
        result_ones = converter.convert(1, curr_in_short, curr_out_short)
        result_ones = str(round(result_ones, 2))
    except RateNotFoundError:
        result_ones = 'Данные по валюте не найдены'
